...
def get_matched_kpis(data, current_start, current_end, compare_start, compare_end):
    ...
    return {
        ...
        'matched_day_count': matched_data['matched_day_count'],
        'expected_day_count': matched_data['expected_day_count'],
        'match_percentage': matched_data['match_percentage']  # Fixed line
    }
...
def main():
    ...
    with header_col1:
        st.markdown(f"""
        <div class="header-title">🌍 {HOTEL_NAME} CELEBRATES EARTH DAY 2025</div>
        """, unsafe_allow_html=True)
    ...
    with row1_cols[3]:
        # Green Champion moved next to Earth Day savings
        st.markdown("""
        <div class="champion-container">
            <img src="https://ui-avatars.com/api/?name=Chinmay&background=10B981&color=fff&size=50" class="champion-photo">
            <div class="champion-info">
    ...
    with row2_cols[1]:
        ...
        st.markdown('<h3 class="feedback-title">Earth Day Feedback & Ideas</h3>', unsafe_allow_html=True)
    ...
    with row3_cols[0]:
        # Energy-saving tips with fixed height and better overflow handling
        st.markdown("""
        <div class="tips-section" style="height: 110px; overflow: hidden;">
            <h3>Quick Energy-Saving Tips</h3>
            <div class="tips-container" style="max-height: 75px; overflow: hidden;">
                <div class="tip-chip">💡 Turn off lights</div>
                <div class="tip-chip">🚿 Shorter showers</div>
                <div class="tip-chip">🌡️ Thermostat 21-23°C</div>
                <div class="tip-chip">☀️ Use natural light</div>
                <div class="tip-chip">🔌 Unplug chargers</div>
                <div class="tip-chip">🚶‍♂️ Use stairs</div>
                <div class="tip-chip">🧺 Re-use towels</div>
                <div class="tip-chip">💧 Report leaks</div>
                <div class="tip-chip">🖨️ Print less</div>
                <div class="tip-chip">🚪 Close doors</div>
                <div class="tip-chip">🔆 Task lighting</div>
                <div class="tip-chip">🌱 Join green team</div>
                <div class="tip-chip">♻️ Recycle</div>
                <div class="tip-chip">🔧 Report issues</div>
                <div class="tip-chip">💻 Power off devices</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    
    # Footer with matched days info
    st.markdown(f"""
    <div class="footer">
        {HOTEL_NAME} Earth Day Challenge | {current_start.strftime('%b %d')} - {current_end.strftime('%b %d, %Y')} | vs {compare_start.strftime('%b %d')} - {compare_end.strftime('%b %d, %Y')} | Based on {kpis['matched_day_count']} matched days out of {kpis['expected_day_count']} expected
    </div>
    """, unsafe_allow_html=True)
